...
export default function AdminSeriesManage() {
  ...
  const { series, loadSeasons, addSeason, updateSeasonCover, deleteSeason, addEpisode, addEpisodesBulk, deleteEpisode } = useLibrary()
  const { push } = useToast()
  const show = series.find((s) => s.folder === folder)

  const [activeSeason, setActiveSeason] = useState(null)
  const [newSeasonNumber, setNewSeasonNumber] = useState('')
  const [newSeasonCover, setNewSeasonCover] = useState(null)
  ...
  async function handleAddSeason(e) {
    ...
    try {
      await addSeason(folder, num, newSeasonCover)
      setActiveSeason(num)
      setNewSeasonNumber('')
  ...
  async function handleSeasonCoverChange(file) {
    if (!activeSeason) return
    try {
      await updateSeasonCover(folder, activeSeason, file)
      push(`Portada de la temporada ${activeSeason} actualizada.`)
    } catch (err) {
      push(err.message || 'No se pudo actualizar la portada.', 'error')
    }
  }
  ...
  async function handleDetectEpisodes(e) {
    ...
    try {
      ...
      // Todo lo detectado se asigna a la temporada activa: el usuario ya
      // está parado en esa pestaña, solo cambia el número de episodio.
      ...
      push(`Se detectaron ${withDefaults.length} episodio(s) para la temporada ${activeSeason}.`)
    } catch (err) {
  ...
  async function handleImportSelected() {
    ...
    setImporting(true)
    try {
      await addEpisodesBulk(folder, activeSeason, rows.map((row) => ({
      ...
      push(`${rows.length} episodio(s) importado(s) en la temporada ${activeSeason}.`)
      setImportRows(null)
      setImportUrl('')
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="season-tabs">
        ...
        <form className="new-season-form" onSubmit={handleAddSeason}>
          ...
          <label className="mini-file-btn">
            {newSeasonCover ? '✓ Portada lista' : '+ Portada (opcional)'}
            <input type="file" accept="image/*" hidden onChange={(e) => setNewSeasonCover(e.target.files?.[0] || null)} />
          </label>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={activeSeason} className="season-panel">
          <div className="season-panel-head">
            ...
            <div className="season-head-actions">
              <label className="mini-file-btn">
                Cambiar portada de la temporada
                <input type="file" accept="image/*" hidden onChange={(e) => e.target.files?.[0] && handleSeasonCoverChange(e.target.files[0])} />
              </label>
          ...
          <div className="import-panel">
            <label>URL de un episodio de esta temporada</label>
            <p className="import-hint">
              Pegá la URL de un capítulo (ej. de archive.org) y se detectan todos los que tengan el mismo
              nombre con distinto número de episodio, asignados directo a la temporada {activeSeason}.
            </p>
            <form className="import-form" onSubmit={handleDetectEpisodes}>
              <input
                value={importUrl}
                onChange={(e) => setImportUrl(e.target.value)}
                placeholder="https://archive.org/download/identificador/archivo-S01E01.mp4"
              />
      ...
      <style>{`
        ...
        .new-season-form { display: flex; gap: 8px; margin-left: 8px; align-items: center; }
        .new-season-form input[type="number"] { width: 110px; }
        .season-panel-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; flex-wrap: wrap; gap: 10px; }
        .season-panel-head h2 { font-size: 16px; margin: 0; }
        .season-head-actions { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }
        .mini-file-btn {
          display: inline-flex; align-items: center; gap: 6px; background: var(--bg-panel-raised);
          border: 1px solid var(--line); color: var(--text-mid); padding: 9px 14px; border-radius: 100px;
          font-size: 12.5px; font-weight: 700; cursor: pointer; white-space: nowrap;
        }
        .mini-file-btn:hover { border-color: var(--blue); color: var(--text-hi); }
        .import-panel { background: var(--bg-panel); border: 1px solid var(--line); border-radius: var(--radius-lg); padding: 18px; margin-bottom: 20px; }
        .import-panel label { margin-bottom: 4px; }
        .import-hint { color: var(--text-mid); font-size: 12.5px; margin: 0 0 12px; line-height: 1.5; }
        ...
        .mini-input { width: 64px; flex-shrink: 0; padding: 8px; }
