...
export function LibraryProvider({ children }) {
  ...
  const addSeason = useCallback(async (folder, seasonNumber, coverFile) => {
    const show = series.find((s) => s.folder === folder)
    ...
    const cover = coverFile ? await resolveImage(coverFile, ['assets', 'seasons'], `${show.slug}-t${seasonNumber}`) : null
    const seasonData = { seasonNumber, episodes: [], cover }
  ...
  const updateSeasonCover = useCallback(async (folder, seasonNumber, coverFile) => {
    const show = series.find((s) => s.folder === folder)
    if (!show) throw new Error('Serie no encontrada')
    const season = show.seasons[seasonNumber] || { seasonNumber, episodes: [] }
    const cover = await resolveImage(coverFile, ['assets', 'seasons'], `${show.slug}-t${seasonNumber}`)
    const nextSeason = { ...season, cover }
  ...
  // Guarda varios episodios de una sola vez (usado por el importador
  // automático). Hacerlo en un solo paso evita que llamadas repetidas a
  // addEpisode se pisen entre sí por trabajar con un estado desactualizado.
  const addEpisodesBulk = useCallback(async (folder, seasonNumber, episodesData) => {
    const show = series.find((s) => s.folder === folder)
    if (!show) throw new Error('Serie no encontrada')
    const season = show.seasons[seasonNumber] || { seasonNumber, episodes: [] }

    const newEpisodes = episodesData.map((data) => ({
      episodeNumber: Number(data.episodeNumber),
      title: data.title || `Episodio ${data.episodeNumber}`,
      description: data.description || '',
      duration: Number(data.duration) || 0,
    ...
    const merged = new Map(season.episodes.map((e) => [e.episodeNumber, e]))
    newEpisodes.forEach((e) => merged.set(e.episodeNumber, e))
    const nextEpisodes = Array.from(merged.values()).sort((a, b) => a.episodeNumber - b.episodeNumber)
    const nextSeason = { ...season, episodes: nextEpisodes }
