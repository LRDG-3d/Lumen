...
export default function Card({ item, type, progress }) {
  ...
  const badgeValue = type === 'series' ? (item.seasonNumbers?.length || 0) : null
  const caption = type === 'series'
    ? `${item.seasonNumbers?.length || 0} temporada${(item.seasonNumbers?.length || 0) !== 1 ? 's' : ''}`
    : (item.year || '')
    ...
    >
      <Link to={to}>
        <div className="poster-frame">
          ...
          {badgeValue !== null && badgeValue > 0 && <span className="corner-badge">{badgeValue}</span>}
      ...
      <style>{`
        ...
        .corner-badge {
          position: absolute; top: 8px; right: 8px; background: var(--yellow); color: #191204;
        ...
        .poster-title { font-size: 13px; margin: 8px 2px 0; color: var(--text-hi); font-weight: 700; }
        .poster-caption { font-size: 11.5px; margin: 2px 2px 0; color: var(--text-low); }
