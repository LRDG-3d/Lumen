...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <style>{`
        .sidebar, .mobile-topbar, .sidebar *, .mobile-topbar * {
        ...
        .side-link { border-radius: var(--radius-sm); overflow: hidden; touch-action: manipulation; outline: none; }
        .side-link-inner {
          position: relative; display: flex; align-items: center; gap: 12px; padding: 13px 12px;
          min-height: 46px; color: var(--text-mid); font-weight: 600; font-size: 14.5px; border-radius: var(--radius-sm);
          transition: color 0.2s;
        }
        .side-link:hover .side-link-inner { color: var(--text-hi); }
        .side-link:active .side-link-inner { transform: scale(0.98); }
        ...
        .hamburger, .close-btn {
          background: none; border: none; color: var(--text-hi); font-size: 20px;
          width: 44px; height: 44px; display: grid; place-items: center; border-radius: 50%;
          touch-action: manipulation; outline: none; margin: -6px;
        }
        .hamburger:active, .close-btn:active { background: rgba(255,255,255,0.08); }
        .admin-pill {
          border: 1px solid var(--yellow); color: var(--yellow); width: 40px; height: 40px; border-radius: 50%;
          display: grid; place-items: center; font-size: 15px; touch-action: manipulation;
        }

        .drawer-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 150; }
        .mobile-drawer {
          position: fixed; top: 0; left: 0; z-index: 160; height: 100dvh;
